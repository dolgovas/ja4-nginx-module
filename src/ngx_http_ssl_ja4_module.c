...
int ngx_ssl_ja4(ngx_connection_t *c, ngx_pool_t *pool, ngx_ssl_ja4_t *ja4)
{
    ...
    /*
    Allocate memory for and populate a list of ja4_ciphers from 'c->ssl->ja4_ciphers'.
    The resulting ja4_ciphers are stored in host byte
    order in 'ja4->ja4_ciphers'. If memory allocation fails, the function returns NGX_DECLINED.
    */
    if (c->ssl->ja4_ciphers && c->ssl->ja4_ciphers_sz)
    {
        ...
        // Add c->ssl->ja4_ciphers to ja4->ciphers
        ...
        /* Now, let's sort the ja4->ja4_ciphers array */
    ...
    if (c->ssl->ja4_extensions_sz && c->ssl->ja4_extensions)
    {
        ...
        for (i = 0; i < c->ssl->ja4_extensions_sz; ++i)
        {
            ...
            {
                char *ext = c->ssl->ja4_extensions[i];
                size_t ext_len = strlen(ext) + 1; // +1 for null terminator

                ja4->extensions_count++;

                // ignored ja4_extensions  are only counted, not hashed
                if (!ngx_ssl_ja4_is_ext_ignored(c->ssl->ja4_extensions[i]))
                {
                    ...
                    if (ja4->ja4_extensions[ja4->ja4_extensions_sz] == NULL)
                    {
                        ...
                        ngx_pfree(pool, ja4->ja4_extensions);
                        ja4->ja4_extensions = NULL;
                        return NGX_DECLINED;
                    }
                ...
                // for no psk ignored ja4_extensions  are not counted, not hashed
                if (ngx_ssl_ja4_is_ext_ignored(c->ssl->ja4_extensions[i]))
                {
                    continue;
                }
                // check if the extension is not a PSK extension
                if (!ngx_ssl_ja4_is_ext_dynamic(c->ssl->ja4_extensions[i]))
                {
                    // Allocate memory for the extension string and copy it
        ...
        /* Now, let's sort the ja4->ja4_extensions  array */
...
{
    // This function calculates the ja4 fingerprint but it doesn't hash ja4_extensions  and ciphers
...
{
    // this function calculates the ja4s fingerprint but it doesn't hash ja4_extensions  and ciphers
    // instead, it just comma separates them
